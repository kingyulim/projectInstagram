...
public class CommentLikeService {
    ...
    public CreateResponse CreateCommentLike(Long commentId, Long userId/*필터로부터 받은 값(수정필요)*/) {
        ...
        if (commentId == null) {throw new CustomException(ExceptionMessageEnum.COMMENT_NOT_FOUND_EXCEPTION);}
        ...
        if (isLiked == true) {
            ...
            if (comment.getUserId().getId().equals(userId)) { //본인이 작성한 게시물과 댓글에 좋아요를 남길 수 없는 기능
